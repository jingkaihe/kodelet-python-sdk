...
async def ask_user_choice(input: AskUserChoiceInput, ctx: ToolContext) -> str:
...
@ext.on("agent.start")
async def notify_ready(_event: AgentStartEvent, ctx: EventContext) -> None:
...
async def approve_bash_command(event: ToolCallEvent, ctx: EventContext) -> EventResult | None:
...
async def read_bash_policy(ctx: EventContext | ToolContext) -> BashPolicy:
    try:
